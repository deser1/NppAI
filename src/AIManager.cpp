...
void sendToCloudBackend(const std::string& jsonPayload) {
    HINTERNET hSession = InternetOpenA("NppAI Plugin", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
    if (hSession) {
        HINTERNET hConnect = InternetConnectA(hSession, "127.0.0.1", 8000, NULL, NULL, INTERNET_SERVICE_HTTP, 0, 1);
        if (hConnect) {
...
std::string AIManager::generateCode(const std::string& prompt, const std::string& currentContext, std::function<void(char, bool)> onToken, std::function<void(int)> onRemove) {
    ...
    if (!currentContext.empty()) {
        formattedPrompt += "[SYSTEM]: Kontekst poprzednich modyfikacji dla tego pliku:\n" + currentContext + "\n\n";
    }
...
void AIManager::startTracking(const std::string& prompt, const std::string& generatedCode, int startLine, int endLine, const std::string& filePath) {
            ...
            while (true) {
                ...
                if (std::chrono::duration_cast<std::chrono::seconds>(now - lastModificationTime).count() >= 5) {
                    ...
                    if (startPos >= 0 && endPos > startPos) {
                        ...
                        jsonPayload += "\"prompt\":\"" + escapeJsonString(lastPrompt) + "\",";
                        jsonPayload += "\"thought_process\":\"" + escapeJsonString(thinkBlock) + "\",";
                        jsonPayload += "\"final_code\":\"" + escapeJsonString(modifiedCode) + "\",";
                        jsonPayload += "\"user_id\":\"programmer_" + std::to_string(GetCurrentProcessId()) + "\"";
                        jsonPayload += "}";
...
void AIManager::checkAndDownloadModelUpdate() {
    std::thread([this]() {
        HINTERNET hSession = InternetOpenA("NppAI Updater", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
        if (!hSession) return;
        ...
        std::string modelPath = getPluginDllDirectory() + "\\models\\NppAI-model-v1.nppai";
        std::string versionPath = getPluginDllDirectory() + "\\models\\NppAI-model-v1.version";
        ...
        HINTERNET hRequest = HttpOpenRequestA(hConnect, "GET", url.c_str(), NULL, NULL, NULL, 0, 1);
        if (hRequest) {
            if (HttpSendRequestA(hRequest, NULL, 0, NULL, 0)) {
                ...
                if (response.find("\"update_available\":true") != std::string::npos || response.find("\"update_available\": true") != std::string::npos) {
                    // Jest aktualizacja! Pobieramy nowy plik modelu
                    HINTERNET hDlRequest = HttpOpenRequestA(hConnect, "GET", "/api/download_model", NULL, NULL, NULL, 0, 1);
                    if (hDlRequest) {
                        if (HttpSendRequestA(hDlRequest, NULL, 0, NULL, 0)) {
                            ...
                            if (outFile.is_open()) {
                                // Pobieramy bezpiecznie w pętli do bufora RAM, unikając problemów z siecią WinINet
                                 char buffer[4096];
                                 DWORD bytesRead = 0;
                                 while (InternetReadFile(hDlRequest, buffer, sizeof(buffer), &bytesRead) && bytesRead > 0) {
                                     outFile.write(buffer, bytesRead);
                                 }
                                outFile.close();
                                
                                // Podmień stary model na nowy (wymaga przeładowania wtyczki by zaczął działać)
