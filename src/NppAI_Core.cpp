...
extern "C" __declspec(dllexport) void beNotified(SCNotification *notifyCode) {
  ...
  case NPPN_READY: {
    // Ustawienie czasu oczekiwania na 600ms, aby uaktywnić zdarzenie
    // "najechaniu myszką"
    ::SendMessage(nppData._scintillaMainHandle, SCI_SETMOUSEDWELLTIME, 600, 0);
    ::SendMessage(nppData._scintillaSecondHandle, SCI_SETMOUSEDWELLTIME, 600,
                  0);
    break;
  }
  case SCN_DWELLSTART: {
    ...
    if (pos != -1) {
      ...
      int selStart =
          (int)::SendMessage(curScintilla, SCI_GETSELECTIONSTART, 0, 0);
      ...
      if (selStart != selEnd && pos >= selStart && pos <= selEnd) {
        const char *plainMsg =
            "NppAI: Kliknij tutaj, aby wyslac zaznaczenie do Chatu";
  ...
  case NPPN_NATIVELANGCHANGED: {
    // Wywoływane, gdy użytkownik zmieni język w ustawieniach Npp
    UpdateLocalization();
    break;
  }
  case NPPN_SHUTDOWN: {
  ...
  case NPPN_FILESAVED: {
    // Kiedy użytkownik zapisze plik, sprawdzamy czy zmodyfikował wygenerowany
    // kod
    int which = -1;
  ...
  case SCN_MODIFIED: {
    if (notifyCode->nmhdr.hwndFrom == nppData._scintillaMainHandle ||
      ...
      if (notifyCode->modificationType &
        ...
        AIManager::getInstance().onEditorModified(
            (HWND)notifyCode->nmhdr.hwndFrom, position, linesAdded);
      }
    }
  } break;
...
// messages : https://github.com/notepad-plus-plus/notepad-plus-plus/issues
//
